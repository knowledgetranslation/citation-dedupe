...
class export(object):
    ...
    def jsonExport(self, inFile=None):
        columns = ', '.join(['%s' % self.mapTagToColumn.get(k, k) for k in self.dataColumns])
        ...
        if self.constraint != '':  # Check does it have any constraints related to Analyzer result table
            ...
            if self.cursor.fetchone():  # If related table exists
                ...
                logging.warning(
                    "WARNING! Couldn't find table %s_entity_map. Please, run Analyser first." % self.tableName)
                return None
        else:
            cur.execute(sql)  # For all data run query
        data = cur.fetchall()

        assert isinstance(cur, object)  # Does Cursor return object
        if cur is not None:
    ...
    def jsonDupesExport(self, inFile='test.json'):
        ...
        sql = "SELECT distinct %(id)s FROM %(tabName)s_entity_map where %(id)s <> canon_id" % {
            'tabName': self.tableName, 'id': self.tablePK}
        ...
        if self.cursor.fetchone():  # If related table exists
            ...
            for row in res:
                sql = "select %(col)s from %(tabName)s t " \
                      ...
                      "where em.%(id)s = %(idVal)s" % {'col': columns, 'idVal': row[self.tablePK],
                                                       'tabName': self.tableName, 'id': self.tablePK}
    ...
    def xmlOriginalExport(self, inFile=None):
        columns = self.originalXml
        sql = "SELECT %s FROM %s %s LIMIT %s" % (columns, self.tableName, self.constraint, self.limit)
    ...
    def exportIntoXmlFile(self):
        res = self.loadData(self.limit)        # Load Data from DB with limitation of output
        data = self.cursor

        if data is not None and res != 0:
            logging.info('Data from Table %s were loaded.' % self.tableName)

            doc = self.generateXML()
            
            if self.writeXmlFile(doc, PATH + self.outFile):
                logging.info('Data were successfully exported into file %s' % self.outFile)
            else:
    ...
    # noinspection PyPep8Naming
    def exportIntoXmlFileFromOriginal(self):
        self.loadData(self.limit)    # Load Data from DB with limitation of output
        data = self.cursor

        if data is not None:  # and self.columns in data.description:
    ...
    def writeXmlFile(self, data, outFile):
        with open(outFile, 'w') as f:
            print(etree.tostring(data, pretty_print=self.prettyPrint, xml_declaration=True, encoding='UTF-8').decode(),
                  file=f)
            return True
    ...
    # noinspection PyPep8Naming,PyPep8Naming,PyPep8Naming,PyPep8Naming,PyPep8Naming,PyPep8Naming,PyPep8Naming,PyPep8Naming,PyPep8Naming,PyPep8Naming,PyPep8Naming,PyPep8Naming
    ...
    def generateXML(self):
        ...
        for record in self.cursor.fetchall():
            ...
            s = self.addElement('source-app', content=self.tagSourceApp['name'],
                                parent=r)  # Add Tag 'source-app' read from parameters
            for k, v in self.tagSourceApp.items():
                s.attrib[k] = v

            rec = {k: v for k, v in record.items() if v is not None and v != ""}

            for k, content in rec.items():  # record.items():

                elementName = self.mapColumnToTag.get(k, k)  # Define element Name as synonym(value) or same(key) from map_column_to_xml dictionary

                # Create tree branches
                if elementName in self.groupedElements:  # If Element should be in Group of elements
                    branch = self.addGroup(elementName, parent=r)  # Create Group Structure
                else:
                    branch = r  # If doesn't have Group than set as Root

                # Add Data leaves into branches
                if content is not None:
                    if elementName in self.listElements:  # For elements within string
                        contents = content.split(self.lineSeparator)  # Create List
    ...
    def getParameters(self, parametersFile):
        ...
        if os.path.exists(parametersFile):
            ...
            with open(parametersFile) as df:
                ...
                self.columns = ', '.join([str(self.mapTagToColumn.get(k, k)) for k in
                                          self.dataColumns])  # List of columns with meaningful Data
...
def start(xmlFile=XML_FILE):
    ...
    if opt:
        ...
        if opt.mode == 'clear':
            export.constraint = ' WHERE %(id)s NOT IN (SELECT %(id)s FROM %(tabName)s_entity_map)' % {
                'id': export.tablePK, 'tabName': export.tableName}
        elif opt.mode == 'dupes':
            export.constraint = ' WHERE %(id)s IN (SELECT %(id)s FROM %(tabName)s_entity_map)' % {'id': export.tablePK,
                                                                                                  'tabName': export.tableName}
        else:
            export.constraint = ''

        if opt.fromOriginal:
            export.columns = export.originalXml
            export.exportIntoXmlFileFromOriginal()  # Export from stored Original XML data
        else:
            # export.columns = export.columns #', '.join(export.columns)  # Generate columns from a list
            export.exportIntoXmlFile()  # Export by generating new XML file
    else:
        initLogging(log_level)  # Start logging
        export.exportIntoXmlFile()  # Export by generating new XML file
